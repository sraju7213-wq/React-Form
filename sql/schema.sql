...
-- price_rules table (discounts/surcharges/multipliers by scope)
...
create policy if not exists "public read cars" on public.cars
  for select using (true);

create policy if not exists "public read price_rules" on public.price_rules
  for select using (true);
